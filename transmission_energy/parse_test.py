...
SYNC_SEQUENCE = 22
BASE_CASES = 11
TRANSMISSION_CASES = 32
TOTAL_CASES = BASE_CASES + TRANSMISSION_CASES
...
def casetxt(x: int) -> str:
  if x == 8:
    ...
    return "Idle"
  elif BASE_CASES < x <= TOTAL_CASES:
    if x%2 == BASE_CASES%2:
       return "BROADCAST - " + tx_power_list[int((x - BASE_CASES - 1) / 2)]
    return "UNICAST - " + tx_power_list[int((x - BASE_CASES - 1) / 2)]
  else:
    return "unknown"
...
#       nrows=10000,
    ...
    head = df['power'].head(int(2*TOTAL_CASES*case_rows))
    ...
    df = df[:-int(len(df.index)-TOTAL_CASES*case_rows)]
    # Generate case indicies
    df['case'] = df.apply(lambda row: (int(row.name/case_rows)%TOTAL_CASES)+1, axis=1)
    df['case_part'] = df.apply(lambda row: int(row.name/(case_rows/3))%3, axis=1)

    if plot:
        # Visualize cases
        for i in range(8,TOTAL_CASES):
            if i%2 == 0:
                visual.append(0.115)
            else:
                visual.append(0.12)
    ...
    transmissions = df.loc[np.where((df['case'].values > BASE_CASES) &
...
if __name__ == "__main__":
    ...
    pathlist = Path("%s/broadcast_unicast/" % dir_path).rglob("*.oml")
