...
def parse(dir_path: str, columns: list, plot: bool):
    ...
    axs[0].plot(tx_power_levels, df['median__mW_mean'], 'o--', label='mean')
    axs[0].plot(tx_power_levels, df['median__mW_max'], 'o--', label='max')
    axs[0].plot(tx_power_levels, df['median__mWh'], 'o--', label='mWh')
    axs[0].set(xlabel="Radio Power in dBm", ylabel="Energy Consumption in Watt")
    ...
    axs[1].set(xlabel="Radio Power in dBm", ylabel="Energy Consumption in Ampere")
...
if __name__ == "__main__":
    ...
    index = ['median__mW_mean', 'median__mA_mean', 'median__mW_max', 'median__mA_max', 'median__mWh', 'median__mAh']

    parse(dir_path, index, plot)
