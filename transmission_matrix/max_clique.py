...
def max_clique(slice: int, matrix: pd.DataFrame, memo_clusters: dict, cluster_set: set, plot: bool=False, debug: bool=False) -> set:
    ...
    while nodes:
        ...
        for i in range(len(nodes)):
            ...
            if plot:
                nodes_in_common = set()
                for common_node in max_cluster:
                    if common_node in memo_clusters[nodes[i][2]]:
                        nodes_in_common.add(common_node)
                        transmission_energy += matrix.loc[nodes[i][2], common_node] + matrix.loc[common_node, nodes[i][2]]
            else:
                nodes_in_common = max_cluster.intersection(memo_clusters[nodes[i][2]])
                for common_node in nodes_in_common:
                    transmission_energy += matrix.loc[nodes[i][2], common_node] + matrix.loc[common_node, nodes[i][2]]
...
def parse(deployment: str, dir_path: Path, power_path: Path, tranmission_energy_path: Path, slice: int, plot: bool=True, debug: bool=False) -> None:
    ...
    if plot:
        ...
        slice = min(slice, len(matrix.index))
        for i in range(1, slice+1):
            start = time.time()
            curr_max_cluster = max_clique(i, matrix, memo_clusters, memo_clusters[max_center_node])
            elapsed_time = time.time() - start
            benchmark_time.append(elapsed_time)
            max_size_per_slice.append(len(curr_max_cluster))

        plt.plot(range(1,len(max_size_per_slice)+1), max_size_per_slice, 'o')
        plt.xticks(range(1,len(max_size_per_slice)+1))
        plt.yticks(range(min(max_size_per_slice), max(max_size_per_slice)+1))
        plt.xlabel('B&B Size')
        plt.ylabel('Max-Clique Size')
        plt.title(deployment.title())
        plt.savefig("%s/plots/%s_max_clique_benchmark.svg" % (dir_path, deployment), format="svg")
        if debug:
            plt.show()
        else:
            plt.close()

        plt.plot(range(1,len(benchmark_time)+1), benchmark_time, 'o')
        plt.xticks(range(1,len(benchmark_time)+1))
        plt.xlabel('B&B Size')
        plt.ylabel('Time to Calculate [s]')
        plt.title(deployment.title())
        plt.savefig("%s/plots/%s_max_clique_time_benchmark.svg" % (dir_path, deployment), format="svg")
        if debug:
            plt.show()
        else:
            plt.close()
        ...
        benchmark_df[deployment.title()] = greedy_time_stats
...
if __name__ == "__main__":
    ...
    power_pathlist = Path("%s/raw_data_old/" % dir_path).rglob("*.log")
