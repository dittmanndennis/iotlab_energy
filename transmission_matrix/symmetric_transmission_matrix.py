...
def parse(deployment: str, dir_path: Path, power_path: Path, tranmission_energy_path: Path):
    ...
    while unconnected_nodes:
        ...
        for node in unconnected_nodes:
            ...
            if len(intermediary_nodes) == 0:
                ...
                if next_matrix.at[node[1], node[0]] > 0:
                    max_energy = max(next_matrix.at[node[0], node[1]], next_matrix.at[node[1], node[0]])
                    
                    next_matrix.at[node[0], node[1]] = max_energy
                    next_matrix.at[node[1], node[0]] = max_energy
    ...
    matrix.to_csv('%s/results/indexed_transmission_matrices/symmetric_sqrd_transmission_matrix_%s.csv' % (dir_path, deployment))
